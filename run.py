...
from src.base import get_filepaths_iterator, get_all_filepaths, get_filename, init_logging
...
from src.config.run import (
    ...
    TEST_NAME,
    # Dataset
    DATASET_IMG_PATH, DATASET_FEAT_PATH,
    DATASET_DATA_COUNT, DATASET_NAME,
    # Model
    MODEL_NAME, MODEL_FAMILY, MODEL, PROCESSOR,
    # Prompts
    PROMPT_PRIMARY, PROMPT_INTER, PROMPT_IS_MULTISTEP,
    PROMPT_PRIMARY_KEY, PROMPT_INTER_KEY,
    # Params
    PARAM_NUM_PER_INFERENCE as NUM, PARAM_USE_NONVIS as NONVIS
)
...
for img_path in tqdm(img_paths):
    ...
    if PROMPT_IS_MULTISTEP:
        inter_output, inter_sec = inference_hf(
            MODEL,
            PROCESSOR,
            PROMPT_INTER.format(number = NUM),
            img_path = img_path
        )
